# تقرير تغطية البيانات

لوحة جودة وتغطية لبيانات الشركات داخل المشروع.

## الملخص التنفيذي

| المؤشر | القيمة |
|---|---|
| الشركات المصنفة | 259 |
| الشركات المؤجلة | 12 |
| نسبة التغطية التقريبية | 103.6% |
| عدد القطاعات | 16 |
| عدد تصنيفات الأعمال | 16 |
| عدد ثيمات رؤية 2030 | 14 |
| unclassified في Vision 2030 | 0 |
| الرموز المكررة | 0 |

## جودة البيانات

| الفحص | القيمة |
|---|---|
| صفوف companies.csv | 259 |
| صفوف companies_classified.csv | 259 |
| صفوف vision2030_themes.csv | 259 |
| تطابق مجموعة الرموز (companies ↔ classified) | yes |
| الرموز المكررة | 0 |
| الرموز الفارغة | 0 |

## مراجعات مفتوحة

- تقرير الشركات المؤجلة: [reports/excluded_or_deferred_companies.md](excluded_or_deferred_companies.md) — 12 شركة مؤجلة
- تقرير الشركات القابضة: [reports/holding_companies_review.md](holding_companies_review.md)
- عدد الشركات القابضة المرشحة: 36
- عدد needs_review في تقرير الشركات القابضة: 8

## توزيع ثيمات رؤية 2030

| الثيم | العدد |
|---|---|
| industrial_development | 65 |
| financial_sector_development | 47 |
| quality_of_life_and_retail | 43 |
| food_security_and_industry | 24 |
| healthcare_quality | 15 |
| housing_and_real_estate | 15 |
| logistics_and_transport | 12 |
| digital_transformation | 11 |
| energy | 10 |
| labor_market_and_services | 9 |
| healthcare_access | 3 |
| utilities_and_infrastructure | 2 |
| food_security_and_retail | 2 |
| renewable_energy_and_utilities | 1 |

## ملاحظات

- لا توجد شركات unclassified بعد التحسين الأخير للتغطية.
- يوجد 12 شركة مؤجلة خارج الإدخال الحالي (موثقة في تقرير الشركات المؤجلة).
- مراجعة الشركات القابضة موجودة كتقرير مستقل ولا تغيّر التصنيف تلقائيًا.

## جودة مصادر البيانات

- official: 259

## نوع المصدر

- official: 259

## التغطية حسب القطاع

- المواد الأساسية: 46
- التأمين: 28
- تجزئة وتوزيع السلع الكمالية: 23
- إنتاج الأغذية: 23
- السلع الرأسمالية: 20
- الخدمات الاستهلاكية: 20
- إدارة وتطوير العقارات: 17
- الرعاية الصحية: 15
- النقل: 12
- الاتصالات: 11
- البنوك: 10
- الخدمات المالية: 10
- الطاقة: 10
- الخدمات التجارية والمهنية: 9
- المرافق العامة: 3
- تجزئة الأغذية: 2

## الشركات التي تحتاج إلى رابط مصدر رسمي


## الشركات التي تحتاج إلى تحقق رسمي

